// query_benchmarker_riakts speed tests RiakTS servers using request
// data from stdin.
//
// It reads encoded HLQuery objects from stdin, and makes concurrent requests
// to the provided RiakTS cluster. This program is a 'heavy client', i.e.
...
// Parse args:
func init() {
	flag.StringVar(&daemonUrl, "url", "localhost:8089", "RiakTS URL.")
...
func main() {
	...
	cluster := NewRiakTSCluster(daemonUrl, requestTimeout)
	...
	qe := NewHLQueryExecutor(cluster, csi, debug)
	for i := 0; i < workers; i++ {
		workersGroup.Add(1)
